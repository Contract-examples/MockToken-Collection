...
abstract contract StandardToken is BasicToken, ERC20 {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public virtual override onlyPayloadSize(3 * 32) {
        ...
        uint256 fee = SafeMath.div(SafeMath.mul(_value, basisPointsRate), 10_000);
        if (fee > maximumFee) {
            fee = maximumFee;
        }
        ...
        uint256 sendAmount = SafeMath.sub(_value, fee);
        balances[_from] = SafeMath.sub(balances[_from], _value);
        balances[_to] = SafeMath.add(balances[_to], sendAmount);
        if (fee > 0) {
...
contract TetherToken is Pausable, StandardToken, BlackList {
    ...
    // Forward ERC20 methods to upgraded contract if this one is deprecated
    function transferFrom(address _from, address _to, uint256 _value) public override(StandardToken) whenNotPaused {
    ...
    // Forward ERC20 methods to upgraded contract if this one is deprecated
    function approve(address _spender, uint256 _value) public override(StandardToken) onlyPayloadSize(2 * 32) {
        if (deprecated) {
    ...
    function totalSupply() public view override(ERC20Basic) returns (uint256) {
        if (deprecated) {
    ...
    function setParams(uint256 newBasisPoints, uint256 newMaxFee) public onlyOwner {
        ...
        maximumFee = SafeMath.mul(newMaxFee, 10 ** decimals);
